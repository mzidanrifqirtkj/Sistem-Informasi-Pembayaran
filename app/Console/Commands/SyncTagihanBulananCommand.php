...
class SyncTagihanBulananCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tagihan:sync-bulanan
     ...
     * @var string
     */
    protected $description = 'Synchronize TagihanBulanan status based on payments';
    ...
    public function handle()
    {
        ...
        $this->info('🔄 Syncing TagihanBulanan status...');
        if ($isDryRun) {
        ...
        try {
            ...
            $totalTagihans = TagihanBulanan::count();
            $this->stats['total'] = $totalTagihans;

            if ($totalTagihans === 0) {
                $this->warn('No TagihanBulanan records found.');
                return 0;
            }
            ...
            TagihanBulanan::with('pembayarans')
            ...
            Log::error('SyncTagihanBulanan command error: ' . $e->getMessage());
            return 1;
        }
    }
     ...
     */
    private function processTagihan(TagihanBulanan $tagihan, bool $isDryRun): void
    {
        try {
            // Calculate total pembayaran
            $totalPembayaran = $tagihan->pembayarans->sum('nominal_pembayaran');

            // Get nominal tagihan (adjust this based on your TagihanBulanan structure)
            ...
            if ($currentStatus === $expectedStatus) {
                ...
                // Log the change
                Log::info('TagihanBulanan status sync', [
            ...
            Log::error('Error processing TagihanBulanan in sync command', [
                'id' => $tagihan->id_tagihan_bulanan,
                'error' => $e->getMessage()
            ]);
        }
    ...
    private function logResults(bool $isDryRun): void
    {
        ...
        Log::channel('single')->info('TagihanBulanan sync completed', $logData);
    }
}
