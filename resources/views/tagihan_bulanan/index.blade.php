            ...
            <div class="card-body p-0">
                                ...
                                <tr>
                                        ...
                                        <div class="monthly-grid">
                                            @foreach (['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'] as $bulan)
                                                ...
                                                <div class="month-status" data-bs-toggle="tooltip"
                                                    ...
                                                    title="{{ $bulan }}: {{ $title }}">
                                                    <i class="{{ $iconClass }}"></i>
                                                    <div class="month-label">{{ substr($bulan, 0, 1) }}</div>
                                                </div>
                                            @endforeach
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-bold">Rp
                                            {{ number_format($santri->total_nominal, 0, ',', '.') }}</span>
                                    </td>
                                    <td class="hide-mobile text-success">
                                        Rp {{ number_format($santri->total_dibayar, 0, ',', '.') }}
                                    </td>
                                    <td>
                                        @if ($santri->total_kekurangan > 0)
                                            <span class="text-danger fw-bold">
                                                Rp {{ number_format($santri->total_kekurangan, 0, ',', '.') }}
                                            </span>
                                        @else
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('tagihan_bulanan.show', ['id' => $santri->id_santri, 'tahun' => $tahun]) }}"
                ...
                @if ($santris->hasPages())
                    <div class="card-footer bg-white">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="text-muted">
                                Menampilkan {{ $santris->firstItem() }} - {{ $santris->lastItem() }} dari
                                {{ $santris->total() }} data
                            </div>
                            <div>
                                {{ $santris->appends(request()->query())->links('pagination::bootstrap-4') }}
                            </div>
                        </div>
                    </div>
                @endif
...
@section('script')
    <script>
        ...
        function exportData() {
            ...
            window.location.href = "{{ route('tagihan_bulanan.export') }}?" + params.toString();
            setTimeout(() => {
