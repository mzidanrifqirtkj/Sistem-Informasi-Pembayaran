    ...
    <div class="container my-4">
        ...
        <div class="row mb-4">
            <div class="col-12 d-flex justify-content-center ">
                <a href="{{ route('ustadz.penugasan.qori.create') }}" class="btn btn-primary">Penugasan Qori</a>
                <a href="{{ route('ustadz.penugasan.mustahiq.create') }}" class="btn btn-primary">Penugasan Mustahiq</a>
            </div>
        </div>
                ...
                <select id="tahunAjar" class="form-control">
                    @foreach ($tahunAjar as $tahun)
                        <option value="{{ $tahun->id_tahun_ajar }}"
                            {{ isset($defaultTahun) && $defaultTahun->id_tahun_ajar == $tahun->id_tahun_ajar ? 'selected' : '' }}>
                            {{ $tahun->tahun_ajar }}
                        </option>
                    @endforeach
                </select>
    ...
    <script>
        $(document).ready(function() {
            ...
            let waliTable = $('#waliTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('ustadz.penugasan.getWaliKelas') }}",
                    type: 'GET',
                    data: function(d) {
            ...
            let qoriTable = $('#qoriTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('ustadz.penugasan.getQori') }}",
                    type: 'GET',
                    data: function(d) {
                        d.id_tahun_ajar = $('#tahunAjar').val();
                    }
